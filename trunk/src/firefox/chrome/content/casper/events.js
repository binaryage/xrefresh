...
Casper.Events.handler = {
    ...
    */
    ...
    blur: function() {
        ...
        this.doEvent = function(e, targetWindow) {
            var evt = Casper.Events.util.deserialize(e, targetWindow.document);
            if (evt.target) return evt.target.blur();
            return null;
        }
    },
    ...
    focus: function() {
        ...
        this.doEvent = function(e, targetWindow) {
            var evt = Casper.Events.util.deserialize(e, targetWindow.document);
            if (evt.target) return evt.target.focus();
            return null;
        }
    },
    ...
    command: function() {
        ...
        this.doEvent = function(e, targetWindow) {
            var evt = Casper.Events.util.deserialize(e, targetWindow.document);
            if (evt.target) return evt.target.doCommand();
            return null;
        }
    },
...
//        Casper.Events.factory.get(e.type).doEvent(e, this.topWindow);
		Casper.Events.factory.get(e.type).doEvent(e, this.targetWindow);
    },
...
Casper.Events.recorder = function(changed) {
    this.onchanged = changed;
    this.log = Casper.Logging.getLogger('Casper::Events::recorder');
    //this.log.setLevel(Casper.Logging.DEBUG); // TODO: remove
    //Casper.Events.log.setLevel(Casper.Logging.DEBUG); // TODO: remove
...
Casper.Events.recorder.prototype = {
    state: "stopped",// "recording", "paused", "replaying"
    currentTest: null,
    listener: null,
    onchange: null,
    
    start: function(aWindow, paused)
    {
		if (this.state=="recording") return;
		if (this.state=="replaying") { this.log("started during replay"); return; }
        ...
        try {
            ...
            this.currentTest = new Casper.Events.test(aWindow);
      		this.currentTest.onchanged = this.onchanged;

            this.listener.start(aWindow);
            Casper.Events.log.debug("start recording");
			this.state="recording";
        } catch(e) { this.log.exception(e); }
		if (paused) this.pause(aWindow);
    },
    
    stop: function(aWindow)
    {
		if (this.state=="stopped") return;
		if (this.state=="replaying") { this.log("stopped during replay"); return; }

        try {
            Casper.Events.log.debug("stop recording");
            this.listener.stop();
            Casper.Events.currentRecorder = null;
			this.currentTest.eventList = [];
	        this.state="stopped";
        } catch(e) { this.log.exception(e); }
        this.log.debug("stop recording with window "+aWindow);
    },
	
	pause: function(aWindow)
	{
		//if (this.state=="paused") return;
        try {
            Casper.Events.log.debug("paused recording");
            this.listener.stop();
            this.state="paused";
        } catch(e) { this.log.exception(e); }
        this.log.debug("paused recording with window "+aWindow);
		
	},
    
	unpause: function(aWindow)
	{
		if (this.state!="paused") { this.log("called unpause where inapropriate"); return; }
        try {
            Casper.Events.log.debug("paused recording");
            aWindow.focus();
            this.listener.start(aWindow);
        ...
        this.log.debug("unpaused recording with window "+aWindow);
	},
	
    replay: function()
    {
		if (this.state=="replaying") { this.log("replay during replay"); return; }
		if (this.state=="recording") { this.log("replay during recording"); return; }
		if (this.state=="stopped") return;
        this.state="replaying";
		if (this.currentTest) this.currentTest.replay();
    },

    stopReplay: function()
    {
		if (this.state!="replaying") { this.log("stopreplay but not replaying"); return; }
        this.state="replaying";
		if (this.currentTest) this.currentTest.cancelReplay = true;
		this.pause();
    },
    ...
    restoreState: function(aWindow)
    {
		if (!this.savedState) { this.log("bad call to restoreState"); return; }
		this.state = "paused";
		if (this.savedState=="recording") this.unpause(aWindow);
    },
    
    handleEvent: function(event)
    {
		if (this.state!="recording") return;
        if (this.currentTest) this.currentTest.addEvent(event);
    },
	
	getStats: function()
	{
		if (!this.currentTest) return "[0]";
		if (this.currentTest.nextEventIndex>0) return "["+this.currentTest.nextEventIndex+"/"+this.currentTest.eventList.length+"]";
		return "["+this.currentTest.eventList.length+"]";
	},
	
	copy: function (source)
	{
		if (!this.currentTest) return;
		if (!source.currentTest) return;
		
        for (var i =0; i < source.currentTest.eventList.length; i++) {
          	this.currentTest.addEvent(source.currentTest.eventList[i]);
        }
	}
