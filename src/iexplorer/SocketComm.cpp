...
#ifdef _UNICODE
		char pstrDevice[HOSTNAME_SIZE];
		WideCharToMultiByte(CP_ACP, 0, strServiceName, -1, pstrDevice, sizeof(pstrDevice), NULL, NULL );
#else
...
bool CSocketComm::CreateSocket(LPCTSTR strServiceName, int nFamily, int nType, UINT uOptions /* = 0 */)
{
	...
	if (INVALID_SOCKET != sock)
	{
		...
		SockAddrIn sockAddr;
		sockAddr.CreateFrom(_T("localhost"), strServiceName, nFamily);
...
*/
		...
		if (SOCKET_ERROR == connect( sock, (SOCKADDR*)sockAddr, sockAddr.Size()))
		{
			closesocket( sock );
			return false;
		}
